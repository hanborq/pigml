...
public class PLSILikelihood extends AbstractStore {
    ...
    public PLSILikelihood() throws IOException {
        ...
        Env.inBackground(new Env.BackgroundProcedure() {
            ...
            public void execute(Configuration conf) throws IOException {
                ...
                Pz = IOUtils.loadMatrix(
                        conf, PathUtils.enter(savedAt, "Pz", DEFAULT_PART)).viewRow(0);
                Pd_z = IOUtils.loadMatrix(conf, PathUtils.enter(savedAt, "Pd_z", DEFAULT_PART));
    ...
    public void putNext(Tuple tuple) throws IOException {
        ...
        for (Vector.Element el : elmnts.nonZeroes()) {
            int w = el.index();
            double p = 0;
            for (int z=0; z<Pz.size(); z++) {
                p += Pz.get(z) * Pw_z.get(w, z) * Pd_z.get(d, z);
            }
            likelihood += el.get() * Math.log(p);
        }
    }
    ...
    public void setStoreLocation(String location, Job job) throws IOException {
        ...
        Preconditions.checkState(
                fs.exists(PathUtils.enter(new Path(location), "Pz", DEFAULT_PART)),
